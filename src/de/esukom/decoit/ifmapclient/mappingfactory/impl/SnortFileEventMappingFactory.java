/*
 * SnortFileEventMappingFactory.java 0.2 13/02/07
 * 
 ...
 */
public class SnortFileEventMappingFactory extends SnortEventMappingFactory {

    // predefined line-positions <-> keys of data inside passed in hashmap-list
     ...
     */
    public SnortFileEventMappingFactory(final Properties props, final ArrayList<HashMap<String, String>> data) {
        super(props, data);
    }
    ...
    protected void createMappingResult(final Properties props, final ArrayList<HashMap<String, String>> res) {
        for (HashMap<String, String> hashMap : res) {
            ...
            Matcher classMatcher = Toolbox.getRegExPattern("regex.class").matcher(hashMap.get(mLineClass));
            ...
            // discovered time
            Matcher timestampMatcher = Toolbox.getRegExPattern("regex.timestamp").matcher(hashMap.get(mLineDiscTime));
            ...
            if (msgType != null && discTime != null && doConvert(msgType)) {
                ...
                Matcher ipMatcher = Toolbox.getRegExPattern("regex.ip4").matcher(hashMap.get(mLineIp));
                if (ipMatcher.find()) {
                    event.setIp(ipMatcher.group());
                    event.setIpType("IPv4");
                }
                else {
                    // if no IPv4-Address, check for IPv6-address, in case that IPv6-Addresses
                    // should also be send to server
                    // TODO: this is currently detected by class-field. should
                    // be integrated into config-file soon(er or later)
                    if (!mSkipIP6) {
                        Matcher ip6Matcher = Toolbox.getRegExPattern("regex.ip6").matcher(hashMap.get(mLineIp));
                        if (ip6Matcher.find()) {
                ...
                if (event.getIp() != null) {
                    ...
                    // set event name
                    Matcher typeMatcher = Toolbox.getRegExPattern("regex.type").matcher(hashMap.get(mLineName));
                    if (typeMatcher.find()) {
                    ...
                    // priority
                    Matcher priorityMatcher = Toolbox.getRegExPattern("regex.priority").matcher(hashMap.get(mLinePrio));
